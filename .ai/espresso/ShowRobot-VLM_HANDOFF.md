...
**Gemma-4 额外**：官方 `chat_template.jinja` 永远拼不出训练分布，vLLM 必须
`--chat-template scripts/gemma4/eval/chat_template_gemma4n_lf.jinja`。详见 GEMMA4_DEBUG.md §3。
...
- ⚠️ `Image features and image tokens do not match` 基本都是**预处理缓存过期** → `overwrite_cache: true`
  或删 `~/.cache/huggingface/datasets/json`。
