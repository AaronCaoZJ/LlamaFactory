#!/usr/bin/env bash
# InternVL3.5-2B LoRA on ms0717_blockpap_follow -- ManiSkill 2 cm atomic-token data built with
...
#
#   bash scripts/internvl/train/ms0717_blockpap_follow_2b.sh          # defaults (GPUs 4,6)
#   GPUS=0,1 bash scripts/internvl/train/ms0717_blockpap_follow_2b.sh # pick GPUs
#
...
#   GPUS=4 FORCE_TORCHRUN=1 bash scripts/internvl/train/ms0717_blockpap_follow_2b.sh
#
# Outputs -> saves/internvl3.5-2b/robot/ms0717_blockpap_follow
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LF_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"

export CONFIG="${CONFIG:-examples/train_lora/internvl/internvl3_5_2b_ms0717_blockpap_follow.yaml}"
