...
```bash
cd scripts/qwen3_5/mikomiko_grok_desc/visualization

FORCE=1 bash build_html.sh          # 全量 120 张 + 基座对照（2 张空闲 H200，约 6 分钟）
bash build_html.sh                  # 复用已有预测，只重建 HTML（不占 GPU，4 秒）
FORCE=1 WITH_BASE=0 bash build_html.sh   # 只跑微调模型，省一半 GPU 时间
```
